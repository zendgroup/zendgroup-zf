...
class HtmlToDoc
{
	...
	{
		$this->docFile = $this->escapeSpencialChar($docfile);
		if(!preg_match("/\.doc$/i",$this->docFile))
			$this->docFile.=".doc";
		return;
	}
	function escapeSpencialChar($string) {
...
EOH;
	...
	function createDoc($html,$file,$download=false)
	{
		...
		if($download)
		{
			@header("Cache-Control: no-cache");// leave blank to avoid IE errors
